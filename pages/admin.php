...
$usersStmt = $pdo->
query('SELECT id, name, email, city, created_at FROM users ORDER BY created_at DESC LIMIT 8'); $recentUsers = $usersStmt->fetchAll(); $listingsStmt = $pdo->query('SELECT listings.id, listings.budget, listings.move_in_date, listings.preferences, listings.created_at, users.name AS owner_name, users.city AS owner_city FROM listings INNER JOIN users ON listings.user_id = users.id ORDER BY listings.created_at DESC LIMIT 8'); $recentListings = $listingsStmt->fetchAll(); $messagesStmt = $pdo->query('SELECT messages.id, messages.message, messages.sent_at, sender.name AS sender_name, receiver.name AS receiver_name FROM messages INNER JOIN users AS sender ON messages.sender_id = sender.id INNER JOIN users AS receiver ON messages.receiver_id = receiver.id ORDER BY messages.sent_at DESC LIMIT 8'); $recentMessages = $messagesStmt->fetchAll(); $reportsStmt = $pdo->query('SELECT r.id, r.reason, r.status, r.created_at, u.name AS reporter_name, l.id AS listing_id FROM listing_reports r INNER JOIN users u ON r.reporter_id = u.id INNER JOIN listings l ON r.listing_id = l.id WHERE r.status = "open" ORDER BY r.created_at DESC LIMIT 8'); $openReports = $reportsStmt->fetchAll(); $verificationStmt = $pdo->query('SELECT vr.id, vr.document_url, vr.note, vr.status, vr.created_at, u.id AS user_id, u.name, u.email FROM verification_requests vr INNER JOIN users u ON vr.user_id = u.id WHERE vr.status = "pending" ORDER BY vr.created_at DESC LIMIT 8'); $pendingVerifications = $verificationStmt->fetchAll(); require_once __DIR__ . '/../includes/header.php'; ?>
  ...
  <div class="col-lg-6">
    <div class="surface-card h-100 p-4">
      ...
      <div class="d-grid gap-3">
        ...
        <?php endif; ?> <?php foreach ($openReports as $report): ?>
        <div class="feature-card p-3">
  ...
  <div class="col-lg-6">
    <div class="surface-card h-100 p-4">
      ...
      <div class="d-grid gap-3">
        ...
        <?php endif; ?> <?php foreach ($pendingVerifications as $item): ?>
        <div class="feature-card p-3">
          ...
          <p class="small copy-muted mt-2 mb-2"><?= e((string)$item['note']) ?></p>
          <?php endif; ?>
          <div class="d-flex gap-2">
            <form method="post" action="../php/admin_action.php" class="m-0">
              <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>" />
