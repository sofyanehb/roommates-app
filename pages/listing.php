...
$stmt = $pdo->
prepare('SELECT l.id, l.user_id, l.budget, l.move_in_date, l.preferences, l.status, l.expires_at, l.image_path, l.created_at, u.name, u.city, u.verification_status, u.plan_tier, u.sleep_schedule, u.smoking_preference, u.pet_preference, u.study_habit FROM listings l INNER JOIN users u ON l.user_id = u.id WHERE l.id = ? LIMIT 1'); $stmt->execute([$listingId]); $listing = $stmt->fetch(); if (!$listing) { set_flash('warning', 'Listing not found.'); redirect('search.php'); } $viewerId = is_logged_in() ? (int)current_user_id() : null; $isOwner = $viewerId !== null && $viewerId === (int)$listing['user_id']; $isAdminViewer = is_logged_in() && is_admin(); if (!$isOwner && !$isAdminViewer) { $isOpen = (string)$listing['status'] === 'open'; $notExpired = empty($listing['expires_at']) || (string)$listing['expires_at'] > date('Y-m-d H:i:s'); if (!$isOpen || !$notExpired) { set_flash('warning', 'This listing is not publicly available.'); redirect('search.php'); } } if ($viewerId !== null && !$isOwner && is_user_blocked($pdo, $viewerId, (int)$listing['user_id'])) { set_flash('warning', 'This listing is unavailable.'); redirect('search.php'); } $relatedSql = 'SELECT l.id, l.user_id, l.budget, l.move_in_date, l.status, u.name, u.verification_status, u.plan_tier FROM listings l INNER JOIN users u ON l.user_id = u.id WHERE u.city = :city AND l.id <> :current_id AND l.status = "open" AND (l.expires_at IS NULL OR l.expires_at > NOW())'; $relatedParams = [ 'city' => (string)$listing['city'], 'current_id' => (int)$listing['id'], ]; if ($viewerId !== null) { $relatedSql .= ' AND l.user_id <> :viewer_id AND NOT EXISTS ( SELECT 1 FROM blocked_users b WHERE (b.user_id = :viewer_blocker AND b.blocked_user_id = l.user_id) OR (b.user_id = l.user_id AND b.blocked_user_id = :viewer_blocked) )'; $relatedParams['viewer_id'] = $viewerId; $relatedParams['viewer_blocker'] = $viewerId; $relatedParams['viewer_blocked'] = $viewerId; } $relatedSql .= ' ORDER BY l.created_at DESC LIMIT 4'; $relatedStmt = $pdo->prepare($relatedSql); $relatedStmt->execute($relatedParams); $relatedListings = $relatedStmt->fetchAll() ?: []; $viewerProfile = []; $isFavorite = false; if ($viewerId !== null) { $profileStmt = $pdo->prepare('SELECT sleep_schedule, smoking_preference, pet_preference, study_habit FROM users WHERE id = ? LIMIT 1'); $profileStmt->execute([$viewerId]); $viewerProfile = $profileStmt->fetch() ?: []; $favoriteStmt = $pdo->prepare('SELECT id FROM favorites WHERE user_id = ? AND listing_id = ? LIMIT 1'); $favoriteStmt->execute([$viewerId, $listingId]); $isFavorite = (bool)$favoriteStmt->fetch(); } $pageTitle = 'Listing Details | Roommates App'; $activePage = 'search'; require_once __DIR__ . '/../includes/header.php'; ?>
  ...
  <div class="row g-4 align-items-center position-relative" style="z-index: 1">
    ...
    <div class="col-lg-4">
      <div class="card-soft p-4">
        <div class="badge-soft text-capitalize mb-2"><?= e((string)$listing['status']) ?></div>
...
<div class="row g-4">
  <div class="col-lg-8">
    <div class="surface-card p-lg-5 h-100 p-4">
      ...
      <img src="<?= e($imgSrc) ?>" alt="Listing image" class="img-fluid mb-4 rounded" style="max-height: 380px; width: 100%; object-fit: cover" />
      <?php endif; ?>
...
<section class="surface-card p-lg-5 mt-4 p-4">
