...
$stmt = $pdo->
prepare('SELECT id, budget, move_in_date, preferences, status, expires_at, image_path, created_at FROM listings WHERE user_id = ? ORDER BY created_at DESC'); $stmt->execute([current_user_id()]); $listings = $stmt->fetchAll(); $unreadNotifications = unread_notification_count($pdo, (int)current_user_id()); $messagesStmt = $pdo->prepare('SELECT m.message, m.sent_at, u.name AS receiver_name FROM messages m INNER JOIN users u ON m.receiver_id = u.id WHERE m.sender_id = ? ORDER BY m.sent_at DESC LIMIT 3'); $messagesStmt->execute([current_user_id()]); $recentMessages = $messagesStmt->fetchAll(); $stats = get_user_overview($pdo, (int)current_user_id()); $stats['city'] = current_user_city(); $stats['name'] = current_user_name(); require_once __DIR__ . '/../includes/header.php'; ?>
    ...
    <div class="surface-card p-lg-5 h-100 p-4">
      ...
      <div class="search-grid">
        ...
        <article class="listing-card p-4">
          ...
          <img src="<?= e($imgSrc) ?>" alt="Listing media" class="img-fluid mb-3 rounded" style="max-height: 210px; object-fit: cover; width: 100%" />
          <?php endif; ?>
  ...
  <div class="col-lg-4">
    <div class="surface-card h-100 p-4">
      ...
      <div class="d-grid gap-3">
        <?php foreach ($recentMessages as $message): ?> <?php $messageText = (string)$message['message']; ?> <?php $messagePreview = function_exists('mb_strimwidth') ? mb_strimwidth($messageText, 0, 120, '...') : (strlen($messageText) > 120 ? substr($messageText, 0, 120) . '...' : $messageText); ?>
        <div class="feature-card p-3">
          <div class="badge-soft mb-2">To <?= e((string)$message['receiver_name']) ?></div>
          <p class="small copy-muted mb-2"><?= e($messagePreview) ?></p>
