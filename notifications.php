...
$stmt = $pdo->prepare('SELECT id, title, body, is_read, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC');
...
<section class="surface-card p-4 p-lg-5">
  ...
  <div class="d-grid gap-3">
    ...
    <?php foreach ($notifications as $notification): ?>
      <article class="feature-card p-3 <?= (int)$notification['is_read'] === 0 ? 'border border-success-subtle' : '' ?>">
