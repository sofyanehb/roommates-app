...
$receiverId = filter_input(INPUT_GET, 'receiver_id', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
$selectedReceiverId = ($receiverId !== false && $receiverId !== null) ? (int)$receiverId : null;

$conversationsStmt = $pdo->prepare('SELECT u.id, u.name, u.city,
...
$conversationsStmt->execute([$currentUserId, $currentUserId, $currentUserId, $currentUserId]);
$conversations = $conversationsStmt->fetchAll();

if ($selectedReceiverId !== null) {
  $participantStmt = $pdo->prepare('SELECT id, name, city FROM users WHERE id = ? LIMIT 1');
  $participantStmt->execute([$selectedReceiverId]);
  $selectedUser = $participantStmt->fetch();

  if (!$selectedUser || is_user_blocked($pdo, $currentUserId, $selectedReceiverId)) {
    set_flash('danger', 'Conversation is unavailable.');
    redirect('chat.php');
  }

  $markReadStmt = $pdo->prepare('UPDATE messages SET is_read = 1 WHERE sender_id = ? AND receiver_id = ? AND is_read = 0');
  $markReadStmt->execute([$selectedReceiverId, $currentUserId]);

  $messagesStmt = $pdo->prepare('SELECT m.id, m.sender_id, m.receiver_id, m.message, m.sent_at,
  ...
  $messagesStmt->execute([$currentUserId, $selectedReceiverId, $selectedReceiverId, $currentUserId]);
      ...
      <div class="card-soft p-3 h-100">
        ...
        <div class="d-grid gap-2">
          ...
          <?php foreach ($conversations as $conv): ?>
            <a href="chat.php?receiver_id=<?= (int)$conv['id'] ?>" class="feature-card p-3 text-decoration-none text-dark <?= $selectedReceiverId === (int)$conv['id'] ? 'border border-success-subtle' : '' ?>">
    ...
    <div class="col-lg-8">
      <div class="card-soft p-3 h-100">
        ...
        <?php else: ?>
          ...
          <div class="d-grid gap-2 mb-3" style="max-height: 420px; overflow-y: auto;">
            <?php if (!$messages): ?>
            ...
            <?php foreach ($messages as $msg): ?>
              ...
              <div class="p-3 rounded <?= $isMine ? 'bg-success-subtle ms-4' : 'bg-light me-4' ?>">
