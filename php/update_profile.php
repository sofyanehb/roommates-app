...
$stmt = $pdo->prepare('UPDATE users SET sleep_schedule = ?, smoking_preference = ?, pet_preference = ?, study_habit = ? WHERE id = ?');
$stmt->execute([$sleep, $smoking, $pet, $study, $userId]);

$_SESSION['user']['city'] = current_user_city();

log_activity($pdo, $userId, 'profile_updated', 'Compatibility preferences updated');
set_flash('success', 'Profile preferences updated.');
redirect('../profile.php');
