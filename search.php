...
$viewerProfile = [];
if ($viewerId !== null) {
  $favoriteStmt = $pdo->prepare('SELECT listing_id FROM favorites WHERE user_id = ?');
  $favoriteStmt->execute([$viewerId]);
  foreach ($favoriteStmt->fetchAll() as $favRow) {
    $favoriteMap[(int)$favRow['listing_id']] = true;
  }
...
<section class="search-grid">
  ...
  <?php if ($listings): ?>
    <div class="surface-card p-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2" data-results-summary>
  ...
  <?php foreach ($listings as $listing): ?>
    <article class="listing-card p-4" data-listing-card data-city="<?= e((string)$listing['city']) ?>" data-budget="<?= e((string)$listing['budget']) ?>">
      ...
      <?php if (!empty($listing['image_path'])): ?>
        <img src="<?= e((string)$listing['image_path']) ?>" alt="Listing image" class="img-fluid rounded mb-3" style="max-height: 220px; object-fit: cover; width: 100%;">
      <?php endif; ?>
